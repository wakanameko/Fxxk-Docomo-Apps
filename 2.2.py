...
# encoded window icon with base32
...
def btn_Disable():
    ...
    osaifuDisable = messagebox.askyesno('確認','おサイフケータイ機能を残しますか？')
...
if ur.release == ('vista' or '7'):label1 = tk.Label(baseGround, text='このソフトフェアを使用するにはADBが必要です。').place(x=253,y=48)
...
elif ur.release == ('8' or '8.1' or '2012ServserR2'):button_adb = tk.Button(
...
button = tk.Button(
    baseGround, text='Uninstall', command=btn_Uninst).place(x= 10, y=10)
button2 = tk.Button(
    baseGround, text='Install', command=btn_Inst).place(x= 90, y=10)
button3 = tk.Button(
    baseGround, text='Disable', command=btn_Disable).place(x= 155, y=10)
button4 = tk.Button(
    baseGround, text='Enable', command=btn_Enable).place(x= 225, y=10)
